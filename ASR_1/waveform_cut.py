...
此module意图把一句话的字和其语音时域wave的位置进行对应
...
def phonemes_frames(fmtx,peak_pos,phonemes_types,continue_k=None):
    '''
    fmtx : frames-matrix ，窗口帧的矩阵 
    phonemes_types: 0:辅 , 1:元  #其实不应该分元辅
    peak_pos: 锋所在的帧 在fmtx中的索引位置
    contunue_k:  None or int ,峰值向后提取的帧的数量
    '''
    wdnum=len(peak_pos)
    fn=len(fmtx)
    rst=[]
    for idx in range(0,wdnum):
        if continue_k ==None:
            ...
            k=continue_k
            consonant_frame=fmtx[max(peak_pos[idx]-2,0):min(peak_pos[idx],fn)] # 2个窗口
            vowel_frame=fmtx[max(peak_pos[idx],0):min(peak_pos[idx]+k,fn)] # 2个窗口
        rst.append(npy.concatenate((consonant_frame,vowel_frame),axis=0))
    return rst


def waveform_cut2wordphonemesdframes(file_name,speech_text,continue_k=None,return_plt_need=False):
    '''
    ...
    word_phonemes_frames=phonemes_frames(fmtx,peak_pos,phonemes_types,continue_k)
...
if __name__=='__main__':
    ...
    # 根据元音位置，结合字的发音，切开有效音频区间为子帧区间 字：[辅音,元音]
    # 元辅音顺序 : 先辅后元：[consonants , vowels],有辅必有元，有元未必有辅
    word_phonemes_frames=phonemes_frames(fmtx,peak_pos,phonemes_types)
    wphf=list(zip(speech_text,word_phonemes_frames))  # word_phonemes_frames
